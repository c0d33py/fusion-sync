...
import { S3FileFieldClient } from "./s3filefield.js";
...
export class FusionSync {
	...
	updateFileElement(fileObject) {
		...
		requestAnimationFrame(() => {
			status.textContent = fileObject.status === this.FILE_STATUS.COMPLETED ? fileObject.status : `${Math.round(fileObject.percentage)}%`;
			status.className = `status ${fileObject.status}`;
			progressBar.style.width = fileObject.percentage + "%";
			progressBar.style.background = fileObject.status === this.FILE_STATUS.COMPLETED ? "green" : fileObject.status === this.FILE_STATUS.FAILED ? "red" : "#222";
		});
	}
	...
	trackUploadedFiles(event) {
		...
		[...targetSelected].forEach(async (file, index) => {
			...
			const fieldValue = await this.s3ffClient.uploadFile(file, this.options.setModelsName);
